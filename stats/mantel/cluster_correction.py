...
for mode in modes:
    for freq in freqs:
        if os.path.isfile(read_dir + mode + '_rthreshold_uncorrected_' + freq + window + cons 
                          + '.mat'):
        ...
        else: print('No uncorrected p-vals < 0.05 for ' + mode + freq)
