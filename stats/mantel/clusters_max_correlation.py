...
for i in range(0, len(isc_files)):
    ...
    print(len(inds_v))
    # isc values at the significant vertices of first cluster
    ...
    plt.box(False)
    plt.grid(False)
    plt.title('n=' + str(len(inds_v)) + ', r=' + str(r_value.round(2)) +
              ', p=' + '{:0.1e}'.format(p_value), y=1.1)
    ...
    # fontsizes end
    plt.show()
    fig.savefig(results_path + '/max_cluster_corr_' + fres[i] + nepsys[i] +
